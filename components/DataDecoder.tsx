...
import { Upload, FileText, Activity, BarChart3, Trash2, ArrowLeft, Database, Search, ChevronRight, Save, MessageSquare, BrainCircuit, Send, Loader2, TrendingUp, Info, Zap, PieChart, ShieldAlert, AlertTriangle, Clock, Battery, Filter, XCircle, History, Gauge, Menu, X, Smartphone, Monitor } from 'lucide-react';
...
const DataDecoder: React.FC<DataDecoderProps> = ({ library, onExit, viewMode, onSwitchMode }) => {
  ...
  return (
    <div className="h-full w-full flex flex-col md:flex-row bg-white overflow-hidden relative">
      {/* System Mode Persistent Sidebar */}
      {!isMobile && (
        <aside className="hidden md:flex w-64 lg:w-72 bg-slate-50 border-r flex-col shrink-0 z-[120]">
          <div className="p-8 border-b bg-white">
            <div className="flex items-center gap-3 mb-1">
              <div className="p-2 bg-slate-900 rounded-xl text-white">
                <Database size={20} />
              </div>
              <h1 className="text-sm font-orbitron font-black text-slate-900 uppercase tracking-tighter">OSM_DECODER</h1>
            </div>
            <p className="text-[8px] font-orbitron font-black text-slate-400 uppercase tracking-widest">v8.4.2_OFFLINE</p>
          </div>
          
          <nav className="flex-1 overflow-y-auto p-4 space-y-1">
            <button 
              onClick={() => fileInputRef.current?.click()}
              className="w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl text-[10px] font-orbitron font-black uppercase text-indigo-600 hover:bg-indigo-50 transition-all text-left mb-4 border border-indigo-100"
            >
            ...
            {menuItems.map(item => (
              <button 
                ...
                className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl text-[10px] font-orbitron font-black uppercase transition-all text-left group disabled:opacity-30 ${tab === item.id ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'text-slate-500 hover:bg-white hover:text-indigo-600 border border-transparent hover:border-slate-200'}`}
              >
                <item.icon size={16} className={tab === item.id ? 'text-white' : 'text-slate-400 group-hover:text-indigo-600'} /> 
                {item.label}
              </button>
            ))}
            ...
            <button 
              ...
              className="w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl text-[10px] font-orbitron font-black uppercase text-slate-500 hover:bg-white hover:text-emerald-600 border border-transparent hover:border-slate-200 transition-all text-left disabled:opacity-30"
            >
              <Save size={16} /> {isExporting ? 'Exporting...' : 'Export CSV'}
            </button>

            <button 
              onClick={() => setShowRangeModal(true)}
              disabled={offlineFrames.length === 0}
              className="w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl text-[10px] font-orbitron font-black uppercase text-slate-500 hover:bg-white hover:text-indigo-600 border border-transparent hover:border-slate-200 transition-all text-left disabled:opacity-30"
            >
              <Filter size={16} /> Set Range
            </button>
          </nav>
          ...
          <div className="p-6 border-t bg-white">
            <button 
              onClick={onExit}
              className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl bg-slate-50 text-slate-500 text-[10px] font-orbitron font-black uppercase hover:bg-slate-100 transition-all active:scale-95"
            >
              <ArrowLeft size={16} /> Exit Decoder
            </button>
          </div>
        </aside>
      )}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="flex items-center gap-2">
            {onSwitchMode && (
              <button 
                ...
                className="p-2 hover:bg-slate-100 rounded-xl transition-all active:scale-95 text-slate-400 hover:text-indigo-600"
                title="Switch View Mode"
              >
                {viewMode === 'mobile' ? <Smartphone size={20} /> : <Monitor size={20} />}
              </button>
            )}
            <button 
              onClick={onExit}
        ...
        {/* Hamburger Menu Sidebar (Mobile or small screens) */}
        {isMenuOpen && (
        ...
        <main className="flex-1 overflow-hidden flex flex-col min-h-0 bg-white relative">
              ...
              <div className="max-w-md w-full p-12 bg-white rounded-[40px] shadow-2xl flex flex-col items-center gap-8 border border-slate-100">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-relaxed">
                    Mission data mapped. Use the navigation to select an analysis dashboard.
                  </p>
                </div>
                <button 
                  onClick={() => {
                    if (isMobile) setIsMenuOpen(true);
                    else setTab('visualizer');
                  }}
                ...
                >
                  <Activity size={20} /> Launch Analysis
                </button>
              </div>
            </div>
          ) : (
        ...
        {isMobile && (
          <nav className="h-16 bg-white border-t flex items-center justify-around px-2 shrink-0 z-[120] safe-pb">
            {menuItems.map(item => (
              <button 
                key={item.id}
                onClick={() => setTab(item.id as any)}
                disabled={offlineFrames.length === 0}
                className={`flex flex-col items-center gap-1 px-3 py-1 transition-all disabled:opacity-30 ${tab === item.id ? 'text-indigo-600' : 'text-slate-400'}`}
              >
                <item.icon size={20} className={tab === item.id ? 'scale-110' : ''} />
                <span className="text-[7px] font-orbitron font-black uppercase tracking-tighter">{item.label}</span>
              </button>
            ))}
          </nav>
        )}
        ...
        {!isMobile && (
          <div className="h-6 bg-slate-100 border-t flex items-center justify-between px-6 text-[8px] font-orbitron font-black text-slate-400 uppercase tracking-widest shrink-0">
             <div className="flex gap-6"><span>{offlineFrames.length.toLocaleString()} Pkts_Buffer</span><span>OSM_TELEMETRY_ENGINE_v9.7</span></div>
